...
local function D_Circle()
    ...
    c.Color = Color3.fromRGB(255, 55, 55)
...
-- ═══════════════════════════════════════════════════
--  ESP DATA
--  _char rastreia qual personagem gerou os drawings.
--  Se o personagem mudar (respawn), destrói e recria.
--  Isso corrige: boxes congeladas ao morrer/respawnar
--  e boxes duplicadas ao trocar de personagem.
-- ═══════════════════════════════════════════════════
local ESPData = {}

local function ESP_Create(plr, char)
    -- mesmo personagem: não recria
    if ESPData[plr] and ESPData[plr]._char == char then return end
    -- personagem diferente ou primeira vez: destrói antigo
    if ESPData[plr] then
...
-- ═══════════════════════════════════════════════════
--  WALLCHECK  (true = livre, false = bloqueado)
-- ═══════════════════════════════════════════════════
...
--  MISC
-- ═══════════════════════════════════════════════════
local SpeedConn, NoclipConn, JumpConn, BHopConn = nil, nil, nil, nil
...
RunService.RenderStepped:Connect(function()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        -- CORREÇÃO: considera morto se Health <= 0 OU se o Humanoid não existe
        local alive = hum ~= nil and hum.Health > 0

        -- ── ESP ──────────────────────────────────────────────
        if C.ESP then
            if not alive or not char then
                -- morto ou sem personagem: esconde TUDO imediatamente
                ESP_Hide(plr)
            else
                -- passa o char atual para detectar respawn
                ESP_Create(plr, char)
                ...
                if dist3D > C.ESP_MaxDist then
                    ...
                    if tl then
                        ...
                        -- fora da tela: esconde mas NÃO destrói (player ainda vivo)
                        ESP_Hide(plr)
                    end
                end
            end
...
-- ═══════════════════════════════════════════════════
--  CLEANUP ESP — ao sair ou respawnar
--  CharacterRemoving garante que o _char antigo seja
--  detectado como diferente na próxima ESP_Create.
-- ═══════════════════════════════════════════════════
Players.PlayerRemoving:Connect(ESP_Remove)

Players.PlayerAdded:Connect(function(plr)
    plr.CharacterRemoving:Connect(function()
        -- esconde imediatamente, o _char antigo ficará
        -- obsoleto e será destruído na próxima ESP_Create
        ESP_Hide(plr)
    end)
end)
...
WinStroke.Color = Color3.fromRGB(180, 30, 30); WinStroke.Thickness = 1.5

local topGrad = Instance.new("Frame", Win)
topGrad.Size = UDim2.new(1,0,0,3); topGrad.BackgroundColor3 = Color3.fromRGB(230,40,40)
topGrad.BorderSizePixel = 0; topGrad.ZIndex = 10
Instance.new("UICorner", topGrad).CornerRadius = UDim.new(0,10)
local tgFix = Instance.new("Frame", topGrad)
tgFix.Size = UDim2.new(1,0,0.5,0); tgFix.Position = UDim2.new(0,0,0.5,0)
tgFix.BackgroundColor3 = Color3.fromRGB(230,40,40); tgFix.BorderSizePixel = 0

local TitleBar = Instance.new("Frame", Win)
TitleBar.Size = UDim2.new(1,0,0,42); TitleBar.Position = UDim2.fromOffset(0,3)
TitleBar.BackgroundColor3 = Color3.fromRGB(14,14,20); TitleBar.BorderSizePixel = 0; TitleBar.ZIndex = 5

local TitleLbl = Instance.new("TextLabel", TitleBar)
TitleLbl.Text = "  ◈  AIO MENU  v4"; TitleLbl.Size = UDim2.new(1,-70,1,0)
TitleLbl.BackgroundTransparency = 1; TitleLbl.TextColor3 = Color3.fromRGB(235,235,245)
...
SubLbl.Text = "ULTRA EDITION"; SubLbl.Size = UDim2.new(1,-70,0,12)
SubLbl.Position = UDim2.new(0,20,1,-13); SubLbl.BackgroundTransparency = 1
SubLbl.TextColor3 = Color3.fromRGB(200,40,40); SubLbl.TextSize = 9
SubLbl.Font = Enum.Font.GothamBold; SubLbl.TextXAlignment = Enum.TextXAlignment.Left; SubLbl.ZIndex = 6

local function TopBtn(lbl, ox, col)
    local b = Instance.new("TextButton", TitleBar)
    b.Size = UDim2.fromOffset(22,22); b.Position = UDim2.new(1,ox,0.5,-11)
...
StatusBar.Size = UDim2.new(1,0,0,22); StatusBar.Position = UDim2.fromOffset(0,45)
StatusBar.BackgroundColor3 = Color3.fromRGB(18,5,5); StatusBar.BorderSizePixel = 0

local StatusLbl = Instance.new("TextLabel", StatusBar)
StatusLbl.Size = UDim2.new(1,-8,1,0); StatusLbl.Position = UDim2.fromOffset(8,0)
StatusLbl.BackgroundTransparency = 1; StatusLbl.TextColor3 = Color3.fromRGB(180,180,190)
...
local function UpdateStatus()
    ...
    if #parts == 0 then
        StatusLbl.Text = "  Nenhuma opção ativa"; StatusLbl.TextColor3 = Color3.fromRGB(100,100,115)
    else
        StatusLbl.Text = "  "..table.concat(parts,"   "); StatusLbl.TextColor3 = Color3.fromRGB(220,80,80)
    end
end
UpdateStatus()

local TabBar = Instance.new("Frame", Win)
TabBar.Size = UDim2.new(1,0,0,32); TabBar.Position = UDim2.fromOffset(0,67)
TabBar.BackgroundColor3 = Color3.fromRGB(14,14,20); TabBar.BorderSizePixel = 0
local tabLine = Instance.new("Frame", TabBar)
tabLine.Size = UDim2.new(1,0,0,1); tabLine.Position = UDim2.new(0,0,1,-1)
tabLine.BackgroundColor3 = Color3.fromRGB(45,45,60); tabLine.BorderSizePixel = 0
...
PageHolder.Name = "PageHolder"; PageHolder.Size = UDim2.new(1,0,0,0)
PageHolder.Position = UDim2.fromOffset(0,99); PageHolder.AutomaticSize = Enum.AutomaticSize.Y
PageHolder.BackgroundTransparency = 1; PageHolder.BorderSizePixel = 0

local Pages, TabBtns, CurTab = {}, {}, nil

local function NewTab(name, icon)
    local btn = Instance.new("TextButton", TabBar)
    btn.Size = UDim2.fromOffset(54,26); btn.BackgroundColor3 = Color3.fromRGB(14,14,20)
    btn.Text = icon.."\n"..name; btn.TextColor3 = Color3.fromRGB(140,140,158)
    btn.TextSize = 9; btn.Font = Enum.Font.GothamBold; btn.BorderSizePixel = 0; btn.LineHeight = 1.1
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0,5)
    local ind = Instance.new("Frame", btn)
    ind.Size = UDim2.new(0.7,0,0,2); ind.Position = UDim2.new(0.15,0,1,-2)
    ind.BackgroundColor3 = Color3.fromRGB(220,40,40); ind.BorderSizePixel = 0; ind.Visible = false
    Instance.new("UICorner", ind).CornerRadius = UDim.new(0,1)
    local page = Instance.new("Frame", PageHolder)
    page.Name = name; page.Size = UDim2.new(1,0,0,0)
    page.AutomaticSize = Enum.AutomaticSize.Y; page.BackgroundTransparency = 1
    ...
    pp.PaddingTop = UDim.new(0,6); pp.PaddingBottom = UDim.new(0,8)
    Pages[name] = page; TabBtns[name] = {btn=btn, ind=ind}
    btn.MouseButton1Click:Connect(function()
        ...
        CurTab = name; page.Visible = true
        btn.TextColor3 = Color3.fromRGB(235,235,245); ind.Visible = true
    end)
    return page
end
...
local function Section(parent, text, order)
    ...
    wrap.Size = UDim2.new(1,0,0,20); wrap.BackgroundTransparency = 1; wrap.LayoutOrder = order
    local line = Instance.new("Frame", wrap)
    line.Size = UDim2.new(1,-80,0,1); line.Position = UDim2.new(0,0,0.5,0)
    line.BackgroundColor3 = Color3.fromRGB(40,40,55); line.BorderSizePixel = 0
    local lbl = Instance.new("TextLabel", wrap)
    lbl.Text = text; lbl.Size = UDim2.new(0,80,1,0); lbl.Position = UDim2.new(1,-82,0,0)
    lbl.BackgroundColor3 = Color3.fromRGB(10,10,15); lbl.TextColor3 = Color3.fromRGB(180,35,35)
    lbl.TextSize = 10; lbl.Font = Enum.Font.GothamBold
    lbl.TextXAlignment = Enum.TextXAlignment.Right; lbl.BorderSizePixel = 0
end

local function Toggle(parent, lbl, order, init, cb)
    local row = Instance.new("Frame", parent)
    row.Size = UDim2.new(1,0,0,30); row.BackgroundColor3 = Color3.fromRGB(18,18,26)
    row.BorderSizePixel = 0; row.LayoutOrder = order
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,6)
    local label = Instance.new("TextLabel", row)
    label.Text = lbl; label.Size = UDim2.new(1,-52,1,0); label.Position = UDim2.fromOffset(10,0)
    label.BackgroundTransparency = 1; label.TextColor3 = Color3.fromRGB(205,205,215)
    label.TextSize = 11; label.Font = Enum.Font.Gotham; label.TextXAlignment = Enum.TextXAlignment.Left
    local track = Instance.new("Frame", row)
    track.Size = UDim2.fromOffset(34,16); track.Position = UDim2.new(1,-42,0.5,-8)
    ...
    thumb.Size = UDim2.fromOffset(12,12); thumb.Position = UDim2.new(0,2,0.5,-6)
    thumb.BackgroundColor3 = Color3.fromRGB(100,100,118); thumb.BorderSizePixel = 0
...
local function Slider(parent, lbl, order, minV, maxV, def, fmt, cb)
    ...
    cont.Size = UDim2.new(1,0,0,46); cont.BackgroundColor3 = Color3.fromRGB(18,18,26)
    cont.BorderSizePixel = 0; cont.LayoutOrder = order
    Instance.new("UICorner", cont).CornerRadius = UDim.new(0,6)
    local label = Instance.new("TextLabel", cont)
    label.Text = lbl; label.Size = UDim2.new(0.62,0,0,17); label.Position = UDim2.fromOffset(9,4)
    label.BackgroundTransparency = 1; label.TextColor3 = Color3.fromRGB(205,205,215)
    label.TextSize = 11; label.Font = Enum.Font.Gotham; label.TextXAlignment = Enum.TextXAlignment.Left
    local valLbl = Instance.new("TextLabel", cont)
    valLbl.Size = UDim2.new(0.38,-9,0,17); valLbl.Position = UDim2.new(0.62,0,0,4)
    valLbl.BackgroundTransparency = 1; valLbl.TextColor3 = Color3.fromRGB(200,40,40)
    ...
    track.Size = UDim2.new(1,-18,0,4); track.Position = UDim2.new(0,9,0,33)
    track.BackgroundColor3 = Color3.fromRGB(36,36,50); track.BorderSizePixel = 0
    Instance.new("UICorner", track).CornerRadius = UDim.new(0,2)
    local fill = Instance.new("Frame", track)
    fill.BackgroundColor3 = Color3.fromRGB(190,32,32); fill.BorderSizePixel = 0
    fill.Size = UDim2.new(0,0,1,0)
    Instance.new("UICorner", fill).CornerRadius = UDim.new(0,2)
    local knob = Instance.new("Frame", fill)
    knob.Size = UDim2.fromOffset(11,11); knob.Position = UDim2.new(1,-5,0.5,-5)
    ...
    hb.Size = UDim2.new(1,0,0,18); hb.Position = UDim2.new(0,0,0,26)
    ...
    RunService.RenderStepped:Connect(function()
        ...
        SetVal(minV + math.clamp((mx - track.AbsolutePosition.X) / track.AbsoluteSize.X, 0, 1) * (maxV - minV))
    end)
    return SetVal
end
...
local function Dropdown(parent, lbl, order, opts, def, cb)
    local cont = Instance.new("Frame", parent)
    cont.Size = UDim2.new(1,0,0,30); cont.BackgroundColor3 = Color3.fromRGB(18,18,26)
    cont.BorderSizePixel = 0; cont.LayoutOrder = order; cont.ClipsDescendants = false
    Instance.new("UICorner", cont).CornerRadius = UDim.new(0,6)
    local label = Instance.new("TextLabel", cont)
    label.Text = lbl; label.Size = UDim2.new(0.55,0,1,0); label.Position = UDim2.fromOffset(10,0)
    label.BackgroundTransparency = 1; label.TextColor3 = Color3.fromRGB(205,205,215)
    label.TextSize = 11; label.Font = Enum.Font.Gotham; label.TextXAlignment = Enum.TextXAlignment.Left
    local selBtn = Instance.new("TextButton", cont)
    selBtn.Size = UDim2.new(0.43,0,0,22); selBtn.Position = UDim2.new(0.56,0,0.5,-11)
    selBtn.BackgroundColor3 = Color3.fromRGB(30,30,45); selBtn.Text = def.."  ▾"
    selBtn.TextColor3 = Color3.fromRGB(200,40,40); selBtn.TextSize = 10
    selBtn.Font = Enum.Font.GothamBold; selBtn.BorderSizePixel = 0
    Instance.new("UICorner", selBtn).CornerRadius = UDim.new(0,5)
    local dd = Instance.new("Frame", cont)
    dd.Size = UDim2.new(0.43,0,0,#opts*22); dd.Position = UDim2.new(0.56,0,1,2)
    dd.BackgroundColor3 = Color3.fromRGB(22,22,32); dd.BorderSizePixel = 0
    dd.Visible = false; dd.ZIndex = 20
    Instance.new("UICorner", dd).CornerRadius = UDim.new(0,5)
    Instance.new("UIStroke", dd).Color = Color3.fromRGB(55,55,75)
    local ddl = Instance.new("UIListLayout", dd); ddl.SortOrder = Enum.SortOrder.LayoutOrder
    for i, opt in ipairs(opts) do
        local ob = Instance.new("TextButton", dd)
        ob.Size = UDim2.new(1,0,0,22); ob.BackgroundTransparency = 1; ob.Text = opt
        ob.TextColor3 = Color3.fromRGB(200,200,210); ob.TextSize = 10
        ob.Font = Enum.Font.Gotham; ob.BorderSizePixel = 0; ob.ZIndex = 21; ob.LayoutOrder = i
        ob.MouseButton1Click:Connect(function() selBtn.Text = opt.."  ▾"; dd.Visible = false; if cb then cb(opt) end end)
    ...
    local open = false
    selBtn.MouseButton1Click:Connect(function() open = not open; dd.Visible = open end)
    return selBtn
end
...
SyncToggle["Aimbot"] = Toggle(PgAim, "Aimbot  (WallCheck ativo)", 2, false, function(on) C.Aimbot = on; UpdateStatus() end)
...
SyncToggle["Trigger"] = Toggle(PgAim, "Triggerbot  (auto click)", 10, false, function(on) C.Trigger = on; UpdateStatus() end)
...
SyncToggle["Silent"] = Toggle(PgSil, "Silent Aim  (hook FireServer)", 2, false, function(on)
    C.Silent = on; UpdateStatus(); EnableSilent(on)
end)
Toggle(PgSil, "Team Check  (Silent)", 3, true, function(on) C.Silent_Team = on end)
local silInfo = Instance.new("TextLabel", PgSil)
silInfo.Text = "⚠  O Silent Aim redireciona projéteis\npara o alvo no FOV sem mover câmera."
silInfo.Size = UDim2.new(1,0,0,36); silInfo.BackgroundColor3 = Color3.fromRGB(30,12,12)
silInfo.TextColor3 = Color3.fromRGB(200,160,100); silInfo.TextSize = 10
silInfo.Font = Enum.Font.Gotham; silInfo.TextWrapped = true
silInfo.TextXAlignment = Enum.TextXAlignment.Left; silInfo.BorderSizePixel = 0; silInfo.LayoutOrder = 4
local sc2 = Instance.new("UIPadding", silInfo); sc2.PaddingLeft = UDim.new(0,8)
Instance.new("UICorner", silInfo).CornerRadius = UDim.new(0,6)

-- ── ABA MISC ──
Section(PgMisc, "MOVIMENTO", 1)
SyncToggle["Speed"] = Toggle(PgMisc, "Speed Hack", 2, false, function(on) C.Speed=on; UpdateStatus(); SetSpeed(on) end)
Slider(PgMisc, "Walk Speed", 3, 16, 350, 60, "%g", function(v) C.WalkSpeed = v end)
SyncToggle["BHop"] = Toggle(PgMisc, "Bunny Hop  (segurar SPACE)", 4, false, function(on) C.BHop=on; UpdateStatus(); SetBHop(on) end)
Section(PgMisc, "PLAYER", 5)
SyncToggle["Noclip"]  = Toggle(PgMisc, "Noclip  (atravessar paredes)", 6, false, function(on) C.Noclip=on; UpdateStatus(); SetNoclip(on) end)
SyncToggle["InfJump"] = Toggle(PgMisc, "Infinite Jump", 7, false, function(on) C.InfJump=on; UpdateStatus(); SetInfJump(on) end)
...
local KbBtns, KbListening = {}, nil
for i, d in ipairs(KbDefs) do
    local row = Instance.new("Frame", PgKeys)
    row.Size = UDim2.new(1,0,0,28); row.BackgroundColor3 = Color3.fromRGB(18,18,26)
    row.BorderSizePixel = 0; row.LayoutOrder = i+1
    Instance.new("UICorner", row).CornerRadius = UDim.new(0,6)
    local lbl = Instance.new("TextLabel", row)
    lbl.Text = d.label; lbl.Size = UDim2.new(1,-105,1,0); lbl.Position = UDim2.fromOffset(10,0)
    lbl.BackgroundTransparency = 1; lbl.TextColor3 = Color3.fromRGB(205,205,215)
    lbl.TextSize = 11; lbl.Font = Enum.Font.Gotham; lbl.TextXAlignment = Enum.TextXAlignment.Left
    local btn = Instance.new("TextButton", row)
    btn.Size = UDim2.fromOffset(88,20); btn.Position = UDim2.new(1,-96,0.5,-10)
    btn.BackgroundColor3 = Color3.fromRGB(26,26,38); btn.TextColor3 = Color3.fromRGB(195,40,40)
    btn.TextSize = 10; btn.Font = Enum.Font.GothamBold; btn.BorderSizePixel = 0
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0,4)
    local function Refresh() btn.Text = "[ "..C[d.id].Name.." ]" end
    Refresh()
    btn.MouseButton1Click:Connect(function()
        if KbListening == d.id then
            KbListening = nil; btn.BackgroundColor3 = Color3.fromRGB(26,26,38); Refresh()
        else
            for _, kb in pairs(KbBtns) do kb.btn.BackgroundColor3 = Color3.fromRGB(26,26,38); kb.refresh() end
            KbListening = d.id; btn.Text = " pressione... "; btn.BackgroundColor3 = Color3.fromRGB(140,22,22)
        end
    end)
    KbBtns[d.id] = {btn=btn, refresh=Refresh}
end
local hint = Instance.new("TextLabel", PgKeys)
hint.Text = "Clique no botão → pressione a tecla  •  ESC cancela"
hint.Size = UDim2.new(1,0,0,24); hint.BackgroundTransparency = 1
hint.TextColor3 = Color3.fromRGB(90,90,110); hint.TextSize = 9
hint.Font = Enum.Font.Gotham; hint.TextWrapped = true; hint.LayoutOrder = 99
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if KbListening then
        if kc == Enum.KeyCode.Escape then
            ...
            C[KbListening] = kc
            if KbBtns[KbListening] then KbBtns[KbListening].btn.BackgroundColor3 = Color3.fromRGB(26,26,38); KbBtns[KbListening].refresh() end
        end
        KbListening = nil; return
    end
    if gp then return end
    local function FireToggle(cfgKey, syncKey, extra)
        local v = not C[cfgKey]; C[cfgKey] = v; UpdateStatus()
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    local tw = TweenService:Create(Win, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
        Size = minimized and UDim2.fromOffset(310,45) or UDim2.fromOffset(310,0)
    })
    TabBar.Visible = not minimized; StatusBar.Visible = not minimized; PageHolder.Visible = not minimized
    if not minimized then Win.AutomaticSize = Enum.AutomaticSize.Y end
    tw:Play()
    tw.Completed:Connect(function() if minimized then Win.AutomaticSize = Enum.AutomaticSize.None end end)
...
CloseBtn.MouseButton1Click:Connect(function()
    ...
    SetSpeed(false); SetNoclip(false); SetInfJump(false); SetBHop(false); EnableSilent(false)
    Screen:Destroy()
end)
